...
//                         // Custom User Agent prevents "Disallowed User Agent" errors
...
//                         // Use a dialog to handle the popup window (for Google Sign-In, etc.)
//                         // This prevents the main WebView from being replaced by the auth flow
//                         if (mounted) {
//                           showDialog(
//                             context: context,
...
class _WebViewScreenState extends State<WebViewScreen> with WidgetsBindingObserver {
  static const platform = MethodChannel('com.dadexpress.restaurant/geolocation');
  InAppWebViewController? _webViewController;
  bool _isLoading = true;
  double _loadingProgress = 0.0;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      debugPrint('📱 App Resumed - Hiding overlay bubble and stopping sound');
      ...
      try {
        ...
        debugPrint('❌ Error stopping ringtone on resume: $e');
      }
        ...
        body: SafeArea(
          child: Stack(
            children: [
                  ...
                  children: [
                    InAppWebView(
                        ...
                        UserScript(
                          source: """
                            (function() {
                              ...
                              // 4. IndexedDB - Removed aggressive mock to let Firebase use real IndexedDB
                              // Modern WebViews support IndexedDB perfectly fine.
                      ...
                      shouldOverrideUrlLoading:
                        ...
                        // 5. External launch check (Phone, WhatsApp, Store, etc.)
                        if (_shouldLaunchExternally(uri)) {
                          await _launchExternalUrl(uri);
                          return NavigationActionPolicy.CANCEL;
                        }

                        // 6. Block Firebase Auth Redirects in Main Frame
                        // This prevents the main app from being replaced by the Firebase Sign-in page
                        // when the native bridge fails or is canceled.
                        if (navigationAction.isForMainFrame &&
                            uri.host.contains('firebaseapp.com') &&
                            (uri.path.contains('/__/auth/') ||
                                uri.toString().contains('auth/handler'))) {
                          debugPrint(
                              '🛑 Blocking main-frame auth redirect to keep user in app: $uri');
                          return NavigationActionPolicy.CANCEL;
                        }
                      ...
                      onWebViewCreated: (controller) async {
                       ...
                       // Native Google Sign-In Javascript Bridge
                        controller.addJavaScriptHandler(
                          ...
                          callback: (args) async {
                            try {
                              debugPrint('🟢 Triggering Native Google Sign In Bridge');
                              
                              // We use the Web Client ID from google-services.json to ensure idToken is generated
                              const String webClientId = '210018822653-6u60b4dlbc7c9p9keb29fvuul8ftfe5q.apps.googleusercontent.com';
                              ...
                              // Force sign out to ensure account picker shows up and we get fresh tokens
                              try {
                                await googleSignIn.signOut();
                              } catch (e) {
                                debugPrint('🟡 Note: Sign out failed (probably wasn\'t signed in): $e');
                              }
                              
                              final GoogleSignInAccount? googleUser = await googleSignIn.signIn();
                              
                              if (googleUser == null) {
                                debugPrint('🟡 Google Sign-In flow canceled by user');
                                return {'success': false, 'error': 'canceled_by_user'};
                              }

                              debugPrint('✅ Native Google Sign In Success: ${googleUser.email}');
                              ...
                              debugPrint('   - idToken: ${idToken != null ? "FOUND (${idToken.length} chars)" : "MISSING ❌"}');
                              debugPrint('   - accessToken: ${accessToken != null ? "FOUND (${accessToken.length} chars)" : "MISSING ❌"}');
                              debugPrint('   - serverAuthCode: ${serverAuthCode != null ? "FOUND" : "MISSING"}');
                              
                              // Return the data exactly as the website expects
                              // We include all common fields used by Firebase and other Auth providers
                              final result = {
                                'success': true, 
                                'idToken': idToken,
                              ...
                              debugPrint('📡 Passing data to web... ${result.keys.join(", ")}');
                              return result;
                              
                            } catch (error) {
                              debugPrint('❌ Native Google Sign-In Bridge Error: $error');
                              return {
                                'success': false, 
                                'error': error.toString(),
                                'details': 'Ensure SHA-1 and Web Client ID are correct in Google Console'
                              };
                            }
                          },
                        );
                        ...
                        // Handler for web buttons that should stop the ringtone (like Reject Order)
                        controller.addJavaScriptHandler(
                          handlerName: 'locationButtonClicked',
                          callback: (args) async {
                            debugPrint('🌐 Web button clicked, stopping ringtone');
                            try {
                              FlutterBackgroundService().invoke('stopRingtone');
                            } catch (e) {
                              debugPrint('❌ Error stopping ringtone from web handler: $e');
                            }
                          },
                        );
                      },
                      ...
                      onLoadStop: (controller, url) async {
                        ...
                        await _injectLinkInterceptorScript(controller);
                        await _injectApiInterceptorScript(controller);
                         await _injectLocationButtonListenerScript(controller);
                      },
                        onProgressChanged: (controller, progress) {
                        setState(() {
                          _loadingProgress = progress / 100;
             ...
             if(Platform.isAndroid) _buildMovableFAB(),     //only for android
            ], 
          ),
        ),
      ),
  ...
  /// Inject JavaScript to detect clicks on web location buttons
  Future<void> _injectLocationButtonListenerScript(
      InAppWebViewController controller) async {
    try {
      const script = r"""
        (function() {
          if (window.__locationButtonListenerInstalled) {
            return;
          }
          window.__locationButtonListenerInstalled = true;
          ...
          function scanAndAttach() {
            ...
            buttons.forEach(function(btn) {
              ...
              if (text.includes('allow location') || 
                  text.includes('enable location') || 
                  text.includes('share location') ||
                  text.includes('reject') ||
                  text.includes('decline') ||
                  text.includes('cancel') ||
                  text.includes('accept') ||
                  text.includes('stop sound')) {
                if (!btn.__locationHandlerAttached) {
                  btn.__locationHandlerAttached = true;
                  btn.addEventListener('click', function() {
                    console.log('🎯 Match found for text: "' + text + '", calling stop audio handler');
                    if (window.flutter_inappwebview && window.flutter_inappwebview.callHandler) {
                      window.flutter_inappwebview.callHandler('locationButtonClicked');
                    }
                  });
                }
              }
      ...
      debugPrint('❌ Failed to inject location button listener script: $e');
    }
  }
