...
//       debugPrint('✅ Accept action clicked from notification');
...
class NotificationService {
  ...
  bool _isInitialized = false;
  static const _platform = MethodChannel('com.dadexpress.restaurant/geolocation');
  ...
  /// Handle notification tap
  void _onNotificationTapped(NotificationResponse response) {
    debugPrint('📱 Local notification tapped: ${response.payload}, Action: ${response.actionId}');
    
    // Stop ringtone if 'Accept' or 'Reject' buttons are pressed in the notification tray
    // The user has set the Reject button ID to 'ok' in showOrderNotification
    if (response.actionId == 'ok' || response.actionId == 'REJECT_ACTION' || response.actionId == 'ACCEPT_ACTION' || response.actionId == 'Reject Order') {
      try {
        FlutterBackgroundService().invoke('stopRingtone');
      } catch (e) {
        debugPrint('❌ Error stopping ringtone on notification action: $e');
      }
    }
  ...
  void _handleNotificationTap(Map<String, dynamic> data) {
    ...
    // Stop any persistent ringtone when navigating to app
    try {
      FlutterBackgroundService().invoke('stopRingtone');
    } catch (e) {
      debugPrint('❌ Error stopping ringtone on tap: $e');
    }
